...
SCHEME="NativeTestHarness"

# Navigate to the test harness
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEST_HARNESS_DIR="$SCRIPT_DIR/../native-tests/ios"
...
# Run codegen to generate Fabric component files
echo ""
echo "=== Running Codegen ==="
(cd ../.. && yarn codegen)

# Configure Xcode project with source files and codegen output
echo ""
echo "=== Configuring Xcode Project ==="
BUNDLE_GEMFILE=../Gemfile bundle exec ruby setup_test_harness.rb
...
DERIVED_DATA_PATH="${XCODE_DERIVED_DATA_PATH:-$TEST_HARNESS_DIR/DerivedData}"
...
if command -v xcbeautify &> /dev/null; then
    xcodebuild build \
        -workspace NativeTestHarness.xcworkspace \
        -scheme "Pods-NativeTestHarness" \
